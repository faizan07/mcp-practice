...
def read_document(
    ...
    if doc_id not in docs:
        raise ValueError(f"Doc with the {doc_id} not found")
...
def edit_document(
    ...
    old_str: str = Field(description="The text to replace. Must match exactly same with the text present in the document"),
...
def fetch_doc(doc_id:str) -> str:
    if doc_id not in docs:
        raise ValueError(f"Document with id {doc_id} not fount")
    return docs[doc_id]
